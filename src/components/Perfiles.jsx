            ...
            <div className="max-w-4xl mx-auto px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="md:col-span-1">
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 sticky top-24 transition-all">
                            ...
                            <form onSubmit={handleGuardar} className="space-y-4">
                                <div>
                                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Alias de la Cuenta (ej: Mercado Pago)</label>
                                    <input
                                        required
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">CBU/Alias</label>
                                    <input
                                        className="w-full px-4 py-2 border border-slate-200 rounded-lg outline-none focus:border-blue-500 transition-all"
                    ...
                    <div className="md:col-span-2 space-y-4">
                        {perfiles.map(p => (
                            <div key={p.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex justify-between items-center group hover:border-blue-200 transition-all">
                                <div className="flex-1">
                                    <h4 className="font-bold text-slate-800 text-lg flex items-center gap-2">
                                        {p.alias}
                                        {p.id === idEnEdicion && <span className="text-[10px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full uppercase tracking-widest">Editando</span>}
                                    </h4>
                                    <p className="text-sm text-slate-500 italic">{p.beneficiario}</p>
                                    <div className="mt-2 flex gap-4 text-xs text-slate-400">
                                        <span className="flex items-center gap-1 font-medium">🏦 {p.banco}</span>
                                        <span className="flex items-center gap-1 font-medium">🔑 {p.cbu_alias}</span>
                                    </div>
                                </div>
