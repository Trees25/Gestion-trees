...
export default function Recibo() {
  ...
  const descargarPDF = () => {
    ...
    doc.text(`Recibí de: ${selectedCliente?.nombre || clienteLibre || "No especificado"}`, 10, 35);
    ...
    autoTable(doc, {
      ...
      startY: 50,
      foot: [[
        { content: 'TOTAL RECIBIDO', colSpan: 3, styles: { halign: 'right', fontStyle: 'bold' } },
        { content: `$${calcularTotal().toFixed(2)}`, styles: { fontStyle: 'bold' } }
      ]],
  ...
  const handleGuardar = async () => {
    if ((!clienteId && !clienteLibre) || !fecha || filas.some(f => !f.descripcion)) {
    ...
    await guardarDocumento({
      ...
      fecha,
      observaciones: clienteLibre ? `Cliente Posventa: ${clienteLibre} | ${observaciones}` : observaciones,
      numero: contador
    });
    navigate("/resumen");
  };
    ...
    <div className="min-h-screen bg-slate-50 pb-12">
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex flex-wrap gap-3 mb-8">
          <button onClick={() => navigate("/admin")} className="px-4 py-2 bg-white border rounded-lg text-sm transition-colors">← Inicio</button>
          <button onClick={() => navigate("/resumen")} className="px-4 py-2 bg-white border rounded-lg text-sm transition-colors">Resumen</button>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border p-8 transition-colors">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-bold text-slate-800 transition-colors">Generar Recibo</h2>
            <div className="bg-slate-100 px-4 py-2 rounded-lg font-bold transition-colors">N° {contador}</div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
              <label className="block text-sm font-medium mb-1 transition-colors">Recibí de *</label>
              ...
              onChange={(e) => {
                ...
                if (s) {
                  if (filas.length === 1 && !filas[0].descripcion) {
                    setFilas([{ descripcion: s.nombre, cantidad: 1, precio_unitario: s.precio }]);
                  } else {
                    setFilas([...filas, { descripcion: s.nombre, cantidad: 1, precio_unitario: s.precio }]);
                  }
                }
              }}
            >
          ...
          <div className="flex justify-between items-center mt-8 pt-8 border-t">
            <div className="text-2xl font-bold">Total: <span className="text-emerald-600">${calcularTotal().toFixed(2)}</span></div>
            <div className="flex gap-3">
              ...
              <button onClick={descargarPDF} className="px-6 py-2 bg-red-600 text-white font-bold rounded-lg shadow-lg">PDF</button>
            </div>
          </div>
        </div>
      </div>
