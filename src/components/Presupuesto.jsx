...
export default function Presupuesto() {
  ...
  const descargarPDF = () => {
    ...
    doc.text(`Cliente: ${selectedCliente?.nombre || "No especificado"}`, 10, 35);
    if (selectedCliente?.dni_cuit) doc.text(`DNI/CUIT: ${selectedCliente.dni_cuit}`, 10, 41);
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-8 transition-colors">
          ...
          <div className="mt-12 pt-8 border-t transition-colors">
            ...
            >
              ...
              {perfilesPago.map(p => <option key={p.id} value={p.id}>{p.alias} - {p.beneficiario}</option>)}
            </select>
          </div>
