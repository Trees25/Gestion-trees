            ...
            <div className="max-w-4xl mx-auto px-4">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-slate-500">Administra tus servicios y precios predefinidos</p>
                    </div>
                </div>

                <div className={`bg-white p-6 rounded-2xl shadow-sm border transition-all ${idEnEdicion ? 'border-amber-300 ring-4 ring-amber-50' : 'border-slate-200'} mb-8`}>
                    ...
                    <form onSubmit={handleGuardar} className="flex flex-col md:flex-row gap-4 items-end">
                        <div className="flex-1 w-full">
                            <label className="block text-sm font-medium text-slate-700 mb-1 font-bold">Nombre</label>
                            <input
                                required
                        ...
                        <div className="w-full md:w-32">
                            <label className="block text-sm font-medium text-slate-700 mb-1 font-bold">Precio</label>
                            <input
                                required
                                type="number"
                                className="w-full px-4 py-2 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-amber-500/20"
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="divide-y divide-slate-100">
                        {servicios.map(s => (
                            <div key={s.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                                <div className="flex-1">
                                    <p className="font-bold text-slate-800 flex items-center gap-2">
                                        {s.nombre}
                                        {idEnEdicion === s.id && <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full uppercase tracking-widest font-bold">Editando</span>}
                                    </p>
                                    <p className="text-amber-600 font-bold text-sm">${Number(s.precio).toFixed(2)}</p>
                                </div>
                        ...
                        {servicios.length === 0 && (
                            <div className="p-12 text-center text-slate-400 italic">No hay servicios en el catálogo</div>
                        )}
                    </div>
                </div>
            </div>
